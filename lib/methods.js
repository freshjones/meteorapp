Meteor.methods({
  ...
  toggleQuoteExtras: function(id, param, value)
  {
	  Service.update( {_id:id }, { $set : { 'extra.includeEnv' : value   } } );
  },
  ...
  featureRebuild: function(quote_id, featureData)
  {
	 
	 //ditch all quote items 
	 Service.update({ _id:quote_id }, { $set : { 'features' : [] } } );
		  
	  //next lets pull in the new sprint data
	  featureData.forEach(function(doc) {
		 Service.update({_id:quote_id }, { $addToSet : { 'features' : doc  } } );
	  });
	  
	  return 'Features Rebuilt';
